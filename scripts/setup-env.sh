...
# deploy-proxy-server.shを更新
sed -i.bak "s/WEB_SERVER_IP=\"\"/WEB_SERVER_IP=\"$WEB_IP\"/" scripts/deploy-proxy-server.sh
sed -i.bak "s/API_SERVER_IP=\"\"/API_SERVER_IP=\"$API_IP\"/" scripts/deploy-proxy-server.sh

# 実行権限を付与
chmod +x scripts/deploy-frontend.sh
chmod +x scripts/deploy-backend.sh
chmod +x scripts/deploy-proxy-server.sh

echo "設定完了！"
echo ""
echo "次のコマンドでデプロイできます："
echo "   ./scripts/deploy-backend.sh       # APIサーバーをデプロイ（踏み台経由）"
echo "   ./scripts/deploy-frontend.sh      # Webサーバーをデプロイ"
echo "   ./scripts/deploy-proxy-server.sh  # Proxyサーバーをデプロイ"
echo ""
